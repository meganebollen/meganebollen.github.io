...
\item
  ...
  \item
    ...
    \end{itemize}
  \item
    Installer le logiciel R sur votre ordinateur en executant le fichier téléchargé.
  \end{enumerate}
\item
...
\tightlist
\item
  Cette zone est dédiée aux fichiers sources. Ce volet permet d'écrire et de sauvegarder les lignes de code. Ce sera la partie la plus utilisé lors de la programmation.
\item
  Le quadrant en haut à droite fournit des informations sur les objets, les variables et les données en mémoire sous l'onglet \texttt{Environment}.
\item
  La console est affichée en bas à gauche. Cette partie permet d'entrer et d'exécuter des instruction en R et voir les résultats s'afficher.
\item
...
\chapter{Objects et opérateurs}\label{objects-et-opuxe9rateurs}

\section{Objects dans R}\label{objects-dans-r}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{summary}\NormalTok{(iris) }\CommentTok{\#donne les résumés numériques des variables en fonction de leur type}
\end{Highlighting}
\end{Shaded}
